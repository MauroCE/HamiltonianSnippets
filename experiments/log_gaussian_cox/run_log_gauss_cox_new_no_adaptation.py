...
if __name__ == "__main__":
    ...
    step_sizes = [0.18]  # np.array(np.geomspace(start=0.001, stop=10.0, num=9, endpoint=True))  # np.array() used only for pylint
    ...
    with open(f"results/new_noadapt_cox{grid_dim}_seed{overall_seed}_N{N}_T{T}_runs{n_runs}_from{eps_to_str(min(step_sizes))}_to{eps_to_str(max(step_sizes))}_skewness{skewness}_aoo{aoo}_skipo{skipo}_adapteps{adapt_epsilon}_adaptT{adapt_T}_mass{mass_params['strategy']}.pkl", "wb") as file:
