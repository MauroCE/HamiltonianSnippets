...
@jit(nopython=True)
def covariance_function(_i, j, i_prime, j_prime, beta, sigma2, dim):
    """Compute the covariance between two points (i, j) and (i', j') on a 2D grid based on the given parameters.

    Parameters:
    - _i, j: Indices of the first point on the 2D grid.
    ...
    """
    return sigma2 * np.exp(-np.sqrt(((_i - i_prime) ** 2) + ((j - j_prime) ** 2)) / (np.sqrt(dim) * beta))


@jit(nopython=True)
def f_covariance_matrix(n_grid, beta, sigma2, dim):
    """Compute the covariance matrix of the Gaussian process on a 2D grid.

    Parameters:
    - n_grid: Number of grid points along one dimension.
    - beta: Scaling factor for the covariance function.
    - sigma2: Variance parameter of the Gaussian process.
    - dim: Total number of grid points (dim = N^2).
    ...
    """
    covariance_matrix = np.zeros((n_grid, n_grid, n_grid, n_grid))
    for _i in range(n_grid):
        for i_prime in range(n_grid):
            for j in range(n_grid):
                for j_prime in range(n_grid):
                    covariance_matrix[_i, j, i_prime, j_prime] = covariance_function(_i, j, i_prime, j_prime, beta, sigma2, dim)
    return covariance_matrix


def make_grid(data, n_grid):
    """Generates a grid from the given data by counting the number of points in each cell of an n_grid x n_grid grid.

    Parameters:
        - data: DataFrame with columns ['data_x', 'data_y'] representing spatial coordinates.
        - n_grid: Number of grid points along one dimension (total grid will be N x N).
    Returns:
        - data_counts: Array of shape (n_grid^2,), the count of points in each grid cell.
    """
    grid = np.linspace(start=0, stop=1, num=n_grid+1)
    dim = n_grid**2
    ...
    for _i in range(n_grid):
        for j in range(n_grid):
            logical_x = (data_x > grid[_i]) & (data_x < grid[_i + 1])
            logical_y = (data_y > grid[j]) & (data_y < grid[j + 1])
            data_counts[_i * n_grid + j] = sum(logical_y & logical_x)
    return data_counts


def create_log_cox_parameters(n_grid):
    ...
    covariance_matrix = f_covariance_matrix(n_grid, beta, sigma2, dim)
    ...
    # Metric tensor
    metric_tensor = deepcopy(inv_covar)  # (dim, dim)
    ...
    parameters = {
        ...
        'Y': make_grid(pd.read_csv('df_pines.csv'), n_grid),  # (dim, )
...
def grad_neg_log_likelihood(x, parameters):
    ...
    gnlls[ok] = - parameters['Y'] + np.exp(x[ok] - 2*np.log(parameters['n_grid']))
    return gnlls
...
def neg_log_likelihood(x, parameters):
    ...
    ok = ok & np.all(x - 2*np.log(parameters['n_grid']) < np.log(np.finfo(np.float64).max), axis=1)
    ...
    nlls[ok] = - np.sum(x[ok] * parameters['Y'] - np.exp(x[ok] - 2*np.log(parameters['n_grid'])), axis=1)
    return nlls
...
if __name__ == "__main__":
    ...
    grid_dim = 400
    params = create_log_cox_parameters(n_grid=int(grid_dim**0.5))
    ...
    step_sizes = [1e-6, 1e-3, 10, 100]  # np.array(np.geomspace(start=0.001, stop=10.0, num=9, endpoint=True))  # np.array() used only for pylint
    ...
    for i in range(n_runs):
        ...
        for eps_ix, eps in enumerate(step_sizes):
            ...
            print(f"\t\tEps: {eps: .7f} \tLogLt: {out['logLt']: .1f} \tFinal ESS: {out['ess'][-1]: .1f}"
                  ...
                  f"{out['epsilon_params_history'][-1][epsilon_params['to_print']]} "
                  f"Final T: {out['T_history'][-1]} Seed {int(seeds[i])} ")
            res.update({'logLt': out['logLt'], 'out': out})
            results.append(res)

    with open(f"results/new_epsdist_cox{grid_dim}_seed{overall_seed}_N{N}_T{T}_runs{n_runs}_from{eps_to_str(min(step_sizes))}_to{eps_to_str(max(step_sizes))}_skewness{skewness}_aoo{aoo}_skipo{skipo}_adapteps{adapt_epsilon}_adaptT{adapt_T}_mass{mass_params['strategy']}.pkl", "wb") as file:
