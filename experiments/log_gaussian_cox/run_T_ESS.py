...
if __name__ == "__main__":
    ...
    skipo = True  # skip overflown trajectories in epsilon computation
    verbose = False
            ...
            for eps_ix, eps in enumerate(step_sizes):
                ...
                try:
                    ...
                    print(f"\t\tEps: {eps: .7f} \tLogLt: {out['logLt']: .1f} \tFinal ESS: {out['ess'][-1]: .1f}"
                          ...
                          f" Epsilon {epsilon_params['to_print'].capitalize()}: {out['epsilon_params_history'][-1][epsilon_params['to_print']]} Seed {int(seeds[i])} ")
    ...
    with open(f"results/T_ESS_cox{grid_dim}_seed{overall_seed}_N{N}_massFalse_runs{n_runs}_from{eps_to_str(min(step_sizes))}_to{eps_to_str(max(step_sizes))}_skewness{skewness}_aoo{aoo}_skipo{skipo}.pkl", "wb") as file:
